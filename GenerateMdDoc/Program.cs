        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                foreach (XmlNode member in members)
                {
                    ...
                    summary = Regex.Replace(summary, @"<see cref=""T:Sisk\.([^""]+)""\s?/>", @"<a href=""/spec/Sisk.$1.md"">Sisk.$1</a>");
                    // replace sisk method properties etc
                    summary = Regex.Replace(summary, @"<see cref=""[^T]:(Sisk\.[a-zA-Z0-9.]+)\.([a-zA-Z0-9.]+)""\s?/>", @"<a href=""/spec/$1.md"">$2</a>");
                    ...
                    else
                    {
                        ...
                        StType parentType = typeList.FirstOrDefault(t => t.FullName == parent);
                        if (parentType.Type != null)
                            parentType.Members.Add(new StMember
                            {
            ...
            foreach (StType type in typeList.OrderBy(t => t.FullName).ToArray())
            {
                ...
                links.Add(new DocLink(type.ShortName, "/spec/" + type.FullName + ".md", type.Type.ToLower()));
                        ...
                        typeFile.AppendLine($"""
                                <tr>
                                    <td style="width: 33%">
                                        ...
                                        <a href="/spec/{member.Filename}.md">
            ...
            foreach (StType type in typeList)
            {
                foreach (StMember member in type.Members)
                {
                    ...
                    md.AppendLine($"""
                            ...
                            Declaring type: [{member.ParentType}](/spec/{member.ParentType}.md) (from {type.Assembly})
            ...
            string specJs = JsonSerializer.Serialize(links, new JsonSerializerOptions
            {
                PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
                WriteIndented = true
            });

            File.WriteAllText("Output/spec.json", specJs);
